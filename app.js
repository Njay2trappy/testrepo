    ...
    const intervalId = setInterval(async () => {
        ...
        try {
            ...
            if (balance >= requiredLamports) {
                ...
                // After successful deposit, generate a new wallet for next deposit
                await generateNewWalletAndNotify(ctx);
                ...
                // After timeout, generate a new wallet for next deposit
                await generateNewWalletAndNotify(ctx);
            }
        } catch (error) {
...
async function generateNewWalletAndNotify(ctx) {
    ...
    await ctx.reply(`A wallet has been generated for you: ${wallet.publicKey.toBase58()}`);
    return wallet;
}

// Handle the /deposit command for user deposit (as an inline button action)
bot.action('start_deposit', async (ctx) => {
    const wallet = await generateNewWalletAndNotify(ctx); // New wallet is created here for each deposit attempt
